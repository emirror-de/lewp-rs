...
impl<Impl: SelectorImpl> SelectorBuilder<Impl> {
    ...
    pub fn build_with_specificity_and_flags(
        ...
        let raw_simple_selectors: *const [Component<Impl>] =
        ...
        let (rest, current) =
            split_from_end(unsafe { &*raw_simple_selectors }, self.current_len);
...
impl<'a, Impl: SelectorImpl> ExactSizeIterator
    for SelectorBuilderIter<'a, Impl>
{
    fn len(&self) -> usize {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if let Some(simple_selector_ref) = self.current_simple_selectors.next()
        {
            ...
            self.combinators.next().map(|(combinator, len)| {
                let (rest, current) =
                    split_from_end(self.rest_of_simple_selectors, len);
...
#[derive(
    Add, AddAssign, Clone, Copy, Default, Eq, Ord, PartialEq, PartialOrd,
)]
struct Specificity {
    id_selectors: u32,
