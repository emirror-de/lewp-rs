...
fn to_ascii_lowercase(s: &str) -> Cow<str> {
    if let Some(first_uppercase) =
        s.bytes().position(|byte| byte >= b'A' && byte <= b'Z')
    {
...
pub trait Parser<'i> {
    ...
    fn parse_pseudo_element(
        ...
        <Self::Impl as SelectorImpl>::PseudoElement,
        ParseError<'i, Self::Error>,
    > {
        Err(location.new_custom_error(
            SelectorParseErrorKind::UnsupportedPseudoClassOrElement(name),
    ...
    Impl::NamespaceUrl: PrecomputedHash,
{
    for component in AncestorIter::new(iter) {
        let hash = match *component {
            ...
            | Component::Namespace(_, ref url) => url.precomputed_hash(),
        ...
        match *self {
            ...
            AttributeInNoNamespaceExists {
                ...
                if !visitor.visit_attribute_selector(
                    &NamespaceConstraint::Specific(&namespace_empty_string::<
                        Impl,
                    >()),
                    local_name,
                    local_name_lower,
                ...
                if !attr_selector.never_matches =>
            {
                ...
                let namespace = match attr_selector.namespace() {
                    ...
                    None => {
                        empty_string =
                            crate::parser::namespace_empty_string::<Impl>();
        ...
        match *self {
            ...
            | NthLastOfType(a, b) => {
                match *self {
                    ...
                    NthLastOfType(_, _) => {
                        dest.write_str(":nth-last-of-type(")?
                    }
                    _ => unreachable!(),
                }
    ...
    Impl: SelectorImpl,
{
    ...
    'outer_loop: loop {
        ...
        let empty =
            parse_compound_selector(parser, &mut state, input, &mut builder)?;
        if empty {
        ...
        if state.intersects(SelectorParsingState::AFTER_PSEUDO) {
            has_pseudo_element =
                state.intersects(SelectorParsingState::AFTER_PSEUDO_ELEMENT);
...
{
    match parse_qualified_name(
        parser, input, /* in_attr_selector = */ false,
    ) {
        Err(ParseError {
        ...
        Ok(OptionalQName::Some(namespace, local_name)) => {
            if state.intersects(SelectorParsingState::AFTER_PSEUDO) {
                return Err(input
                    .new_custom_error(SelectorParseErrorKind::InvalidState));
            }
            match namespace {
    ...
    Impl: SelectorImpl,
{
    ...
    match parse_qualified_name(
        parser, input, /* in_attr_selector = */ true,
    )? {
        ...
        OptionalQName::Some(ns, Some(ln)) => {
            ...
            namespace = match ns {
                QNamePrefix::ImplicitNoNamespace
                | QNamePrefix::ExplicitNoNamespace => None,
                QNamePrefix::ExplicitNamespace(prefix, url) => {
    ...
    let operator = match input.next() {
        ...
        Err(_) => {
            let local_name_lower =
                to_ascii_lowercase(&local_name).as_ref().into();
    ...
    let value = match input.expect_ident_or_string() {
        ...
        Err(BasicParseError {
            ...
            return Err(location
                .new_custom_error(SelectorParseErrorKind::BadValueInAttr(t)))
        }
    ...
    let case_sensitivity;
    {
        ...
        case_sensitivity = attribute_flags.to_case_sensitivity(
            local_name_lower_cow.as_ref(),
            namespace.is_some(),
        );
        local_name_lower = local_name_lower_cow.as_ref().into();
        local_name_is_ascii_lowercase =
            matches!(local_name_lower_cow, Cow::Borrowed(..));
    }
    let local_name = local_name.as_ref().into();
...
fn parse_attribute_flags<'i, 't>(
    ...
    let ident = match *token {
        Token::Ident(ref i) => i,
        ref other => {
            return Err(location.new_basic_unexpected_token_error(other.clone()))
        }
    };
        ...
        if empty {
            if let Some(url) = parser.default_namespace() {
                ...
                if !ignore_default_ns {
                    builder
                        .push_simple_selector(Component::DefaultNamespace(url));
                }
            }
        }
    ...
    Impl: SelectorImpl,
{
    ...
    Ok(Some(match token {
        ...
        Token::Colon => {
            let location = input.current_source_location();
            let (is_single_colon, next_token) =
                match input.next_including_whitespace()?.clone() {
                    Token::Colon => {
                        (false, input.next_including_whitespace()?.clone())
                    }
                    t => (true, t),
                };
                        ...
                        && name.eq_ignore_ascii_case("part")
                    {
                        ...
                        let names = input.parse_nested_block(|input| {
                            ...
                            while !input.is_exhausted() {
                                result.push(
                                    input.expect_ident()?.as_ref().into(),
                                );
                            }
                            Ok(result.into_boxed_slice())
                        })?;
                        return Ok(Some(
                            SimpleSelectorParseResult::PartPseudo(names),
                        ));
                    }
                    if P::parse_slotted(parser)
    ...
    Impl: SelectorImpl,
{
    if !state.allows_non_functional_pseudo_classes() {
        return Err(
            location.new_custom_error(SelectorParseErrorKind::InvalidState)
        );
    }
...
pub mod tests {
    ...
    fn parse_ns_expected<'i, 'a>(
        ...
        let result =
            SelectorList::parse(parser, &mut CssParser::new(&mut parser_input));
    ...
    fn test_parsing() {
        ...
        assert_eq!(
            parse_ns(
                "*|e",
                &DummyParser::default_with_namespace(DummyAtom::from(
                    "https://mozilla.org"
                ))
            ),
            Ok(SelectorList::from_vec(vec![Selector::from_vec(
        ...
        assert!(parse(":not(#provel > old)").is_ok());
        assert!(
            parse("table[rules]:not([rules=\"none\"]):not([rules=\"\"])")
                .is_ok()
        );
